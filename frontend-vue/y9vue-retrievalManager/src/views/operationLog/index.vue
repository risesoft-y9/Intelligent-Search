                    ...
                    <template v-slot:slotSearch>
                        <el-divider content-position="center">
                            ...
                            <el-button
                                ...
                                @click="reset()"
                            >
                                <i class="ri-restart-line"></i>{{ $t('重置') }}
                            </el-button>
                        </el-divider>
                    </template>
                </y9Table>
...
<script lang="ts" setup>
    ...
    import { searchUserLogList } from '@/api/data/index';
    ...
    let operationLogsTable = ref({
        columns: [
            ...
            { title: computed(() => t('操作用时')), key: 'elapsedTime', width: 150 },
            {
                title: computed(() => t('操作时间')),
                key: 'logTime',
                width: settingStore.getDatetimeSpan,
            },
            { title: computed(() => t('参数')), key: 'parameter' },
        ],
        tableData: [],
        pageConfig: {
    ...
    async function getDataList() {
        ...
        let params = {
            ...query.value,
            page: operationLogsTable.value.pageConfig.currentPage,
            limit: operationLogsTable.value.pageConfig.pageSize,
        };
        let res = await searchUserLogList(params);
        if (res.code == 0) {
            operationLogsTable.value.tableData = res.data.dataList;
            operationLogsTable.value.pageConfig.total = res.data.total;
        }
