    ...
    let typeTableConfig = ref({
        ...
        columns: [
            ...
            { title: computed(() => t('同步错误原因')), key: 'error', width: 180 },
            {
                title: computed(() => t('状态')),
                render: (row) => {
                    return row.parsingFile == 'parse' && row.parsingDocment == 'parse'
                        ? h('span', { style: { color: 'green' } }, 'success')
                        : h('span', { style: { color: 'red' } }, 'false');
                },
            },
            {
                title: computed(() => t('操作')),
                    ...
                    let button = [
                        h(
                            'span',
                            {
                                ...
                                onclick: async () => {
                                    ...
                                    fileListId.value = row.id;
                                    dialogTableConfig.value.loading = true;
                                    await getAnnexList({ id: row.id })
                                        .then((result) => {
                                            // 接口返回正确
                                            if (result.code == 0) {
                                                // 列表数据赋值
                                                dialogTableConfig.value.tableData = result.data.dataList;
                                            }
                                        })
                                        .catch(() => {});
                                    dialogTableConfig.value.loading = false;
                                },
                            },
                            [
                                h('i', {
                        ...
                        h(
                            'span',
                            {
                                style: { display: 'inline-flex', alignItems: 'center' },
                                onclick: () => {
                                    ElMessageBox.confirm(t('是否确认解析'), t('提示'), {
                                        ...
                                        .then(async () => {
                                            loading.value = true;
                                            await parseMainText({ id: row.id })
                                                .then((result) => {
                                                    if (result.code == 0) {
                                                        row.parsingDocment = 'parse';
                                                    }
                                                    // 提示信息
                                                    ElNotification({
                                                        title: result.code == 0 ? t('成功') : t('失败'),
                                                        message: result.code == 0 ? t('解析正文成功') : result.msg,
                            ...
                            'span',
                            {
                                ...
                                onclick: () => {
                                    ElMessageBox.confirm(t('是否确认解析'), t('提示'), {
                                        ...
                                        .then(async () => {
                                            loading.value = true;
                                            await parseAnnex({ id: row.id })
                                                .then((result) => {
                                                    if (result.code == 0) {
                                                        row.parsingFile = 'parse';
                                                    }
                                                    // 提示信息
                                                    ElNotification({
                                                        title: result.code == 0 ? t('成功') : t('失败'),
                                                        message: result.code == 0 ? t('解析附件成功') : result.msg,
                                                        type: result.code == 0 ? 'success' : 'error',
                                                        duration: 2000,
                                                        offset: 80,
                                                    });
